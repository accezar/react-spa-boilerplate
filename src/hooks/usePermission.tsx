 ...
 * usePermission hook
 *
 * Checks if inside a roles object
 * matchs ALL THE ROLES you want to check
 *
...
export function usePermission(
  ...
  if (userPermissions && userPermissions.length !== 0 && rolesToCheck.length !== 0) {
    return userPermissions.filter(item => roles.includes(item.name)).length === rolesToCheck.length;
  }
  return false;
}
